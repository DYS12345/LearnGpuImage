...
@property (weak, nonatomic) IBOutlet GPUImageView *currentImageView;
...
-(void)laShengFilter{
//    拉升变形镜滤镜
    UIImage *inputImage = [UIImage imageNamed:@"dong"];
    ...
    self.nImageView.image = currentFilteredVideoFrame;
}

-(void)fuHeFilter{
    //复合滤镜
    //获取原图
    GPUImagePicture *gpupicture = [[GPUImagePicture alloc] initWithImage:[UIImage imageNamed:@"dong"] smoothlyScaleOutput:YES];
    GPUImageSmoothToonFilter *toonFilter = [GPUImageSmoothToonFilter new];
    ...
    GPUImageFilterPipeline *pipLine = [[GPUImageFilterPipeline alloc] initWithOrderedFilters:filters input:gpupicture output:self.currentImageView];
...
-(void)blackAndWhiteFilter{
    ...
    [disFilter addTarget:self.currentImageView];
    [stillImageSource processImage];
}
...
-(void)customFilter{
    ...
    [customFilter addTarget:self.currentImageView];
    
    [stillImageSource processImage];
    
    UIImage *image = [customFilter imageFromCurrentFramebuffer];
